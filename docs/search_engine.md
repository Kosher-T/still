...
- Upon timeout, the system executes a **partial search** on whatever words are in the buffer (even if fewer than 15).
- Immediately after evaluation, the buffer is **completely flushed**. Retaining stale text across a long pause and concatenating it with newly resumed speech would mathematically destroy the geometric vector coordinates generated by the semantic model.
- The RRF scoring is adjusted via **Dynamic RRF Scaling** (see Phase 3 below).
